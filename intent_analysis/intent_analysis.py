...
def convert_example(example, tokenizer, label_map, max_length=128):
   ...
   labels = [1 if label in example['labels'] else 0 for label in label_map]
   return encoded_input['input_ids'], encoded_input['token_type_ids'], encoded_input['attention_mask'], labels
...
# 定义模型
class BertForMultiLabelSequenceClassification(paddle.nn.Layer):
...
model = BertForMultiLabelSequenceClassification(num_classes=len(label_map))
...
def train(model, data_loader, optimizer, criterion, epochs=3):
   ...
   for epoch in range(epochs):
       ...
       for batch in data_loader:
           ...
           loss = criterion(logits, paddle.to_tensor(labels, dtype='float32'))
           loss.backward()
           optimizer.step()
...
def evaluate(model, data_loader, label_map):
   ...
   with paddle.no_grad():
       for batch in data_loader:
           ...
           total_correct += (predictions == paddle.to_tensor(labels)).sum().item()
...
def predict(text, model, tokenizer, label_map, max_length=128):
   ...
   # 将预测的整数标签转换回原始标签
   predicted_labels = [label for label, idx in label_map.items() if predictions[0][idx].item() == 1]
...
# 保存模型
paddle.save(model.state_dict(), 'bert_multi_intent_recognition_model.pdparams')

# 加载模型
model = BertForMultiLabelSequenceClassification(num_classes=len(label_map))
model.load_dict(paddle.load('bert_multi_intent_recognition_model.pdparams'))
model.eval()
